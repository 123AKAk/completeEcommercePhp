...
if(isset($_POST['form_add_to_cart'])) {
    ...
    try 
    {
        ...
        $statement->execute(array($_POST['eypid']));
	...
	if($_POST['p_qty'] > $current_p_qty)
    {
        ...
        echo json_encode(['response' => "none", 'message' => 'Sorry! There are only '.$current_p_qty.' item(s) in stock']);
    }
	else
    {
        if(isset($_SESSION['cart_p_id']))
        {
            ...
            {
                if( ($arr_cart_p_id[$i]==$_POST['eypid']) && ($arr_cart_size_id[$i]==$size_id) && ($arr_cart_color_id[$i]==$color_id) ){
                    $added = 1;
                    break;
                }
            }
            if($added == 1) {
                ...
                echo json_encode(['response' => "none", 'message' => 'This product is already added to the shopping cart.']);
            }
            else
            {
                ...
                $_SESSION['cart_p_id'][$new_key] = $_POST['eypid'];
                ...
                echo json_encode(['response' => true, 'message' => 'Product is added to the cart successfully!']);
            }
        ...
        else
        {
            ...
            $_SESSION['cart_p_id'][1] = $_POST['eypid'];
...
{
    echo json_encode(['response' => false, 'message' => 'Authentication Error']);   
}
